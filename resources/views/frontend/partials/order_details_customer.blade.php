<div class="modal-header">
    ...
    <button type="button" class="close btn btn-close" data-bs-dismiss="modal" aria-label="Close" style="min-width: 25px;">
    </button>
</div>
...
<div class="modal-body gry-bg px-3 pt-0">
    <div class="pt-4">
        <ul class="process-steps c-black clearfix">
                                    ...
                                    <td>
                                        @if ($orderDetail->product != null)
                                            {{--<a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank">{{ $orderDetail->product->name }}</a>--}}
                                          	@if (strpos($orderDetail->product->name, 'SEO') !== false)
                                         	<a href="{{ route('seo') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                          	@elseif (strpos($orderDetail->product->name, 'WDD') !== false)
                                      		<a href="{{ route('wdd') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                      		@elseif (strpos($orderDetail->product->name, 'Web') !== false)
                                      		<a href="{{ route('wdd') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                      		@elseif (strpos($orderDetail->product->name, 'ORM') !== false)
                                      		<a href="{{ route('orm') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                      		@elseif (strpos($orderDetail->product->name, 'PPC') !== false)
                                      		<a href="{{ route('ppc') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                      		@elseif (strpos($orderDetail->product->name, 'EM') !== false)
                                      		<a href="{{ route('em') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                      		@elseif (strpos($orderDetail->product->name, 'Social') !== false)
                                      		<a href="{{ route('social') }}" target="_blank">{{ $orderDetail->product->name }}</a>
                                      		@endif
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    {{--<strong><span>{{ single_price($order->grand_total) }}</span></strong>--}}
                                  	<strong><span>{{ single_price($order->orderDetails->sum('price')) }}</span></strong>
