...
class HomeController extends Controller
{
    ...
    //     $sort_by_alphabet = $request->sort_by_alphabet;
    //     //$sort_by_price = $request->price;
    ...
    //         array_push($selected_categories_ids, $request['categories']);
            
    //     }
        
    //     if($topics != null){
    //         $products = $products->where(function($query) use ($topics){
    ...
    //     $all_tags = array('Plugin','Wordpress','Tips & Tricks','Development','Design','HTML');
    //     $selected_tags = array();
    //     array_push($selected_tags, $request['topics']);
        
        
    //     if($sort_by_alphabet != null){
    //         switch ($sort_by_alphabet) {
    //             case '1':
    ...
    //     // //foreach ($all_tags as $attribute) {
    //     //     if($request->has('topics')){
    //     //         $loop_index = 1;
    //     //         foreach ($request['topics'] as $value) {
    //     //             $str = '"'.$value.'"';
    //     //             if ($loop_index == 1){
    //     //                 $qry = "->where('tags', 'like', '%'.$str.'%')";
    //     //             }
    //     //             else{
    //     //                 $qry .= "->orWhere('tags', 'like', '%'.$str.'%')";
    //     //             }    
    //     //             $loop_index++;
    //     //         }
                
    //     //         $products = $products.$qry;
                
    //     //         $item['values'] = $request['topics'];
    //     //         array_push($selected_tags, $item);
    //     //     }
    //     // //}

    //     $products = filter_products($products)->paginate(12)->appends(request()->query());

    //     //return view('frontend.product_listing', compact('products', 'query', 'category_id', 'subcategory_id', 'subsubcategory_id', 'brand_id', 'sort_by_alphabet', 'sort_by_price', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color'));
    //     return view('frontend.product_listing', compact('products', 'sort_by_alphabet', 'sort_by_price', 'categories', 'selected_categories_ids', 'all_tags', 'selected_tags'));
    // }
    ...
    public function ldd()
    {
        return view('frontend.ldd');
    }
    
    public function consulting_services()
    {
        return view('frontend.consulting_services');
    }
    ...
    public function wdd()
    {
        return view('frontend.wdd');
    }
    public function em()
    {
        return view('frontend.em');
    }
    
    public static function tags_count($tags)
    {
        $tag_count = 0;
        
        $products = Product::where('published', 1)->where('tags', 'like', '%'.$tags.'%')->get();
        
        if($products!=null && count($products) ){
            $tag_count = count($products);
        }
        return $tag_count;
    }
