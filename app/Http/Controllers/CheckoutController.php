...
class CheckoutController extends Controller
{
    ...
    {   
      //dd($request);
        ...
        if($request->session()->get('order_id') != null){
            ...
            $request->payment_option = 'citigate';
            
            if ($request->payment_option == 'citigate') {
                $citigate = new CitigateController;
                return $citigate->index($request);
            }
    ...
    public function checkout_done($order_id, $payment)
    {
        ...
        /******************email after payment ****************************************/     
        $Customer_email = json_decode($order->shipping_address)->email;
        ...
        $array['order'] = $order;
      	...
      	if(env('MAIL_USERNAME') != null){
                try {
                    Mail::to($Customer_email)->queue(new InvoiceEmailManager($array));
                    Mail::to(User::where('user_type', 'admin')->first()->email)->queue(new InvoiceEmailManager($array));
                  	//Mail::to('support@oceanbluemarketing.com')->queue(new InvoiceEmailManager($array));
                } catch (\Exception $e) {
					dd($e);
                }
            }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        $data['shipping_type'] = 'home_delivery'; //added by gopal
        ...
        //dump($request->session()->get('shipping_info'));
        //dd($total);
        
        
        //$cart = $request->session()->get('shipping_info', collect([]));
        //dd($cart);

        //return view('frontend.delivery_info'); // commented by gopal 03jul2020
        
        // return view('frontend.payment_select', compact('total'));
        
        $this->store_delivery_info($request);
        
        //return view('frontend.payment_select', compact('total')); //commented on 9 jul2020
        /* 10 jul */
    ...
    //public function store_delivery_info(Request $request) coomented by gopal 03jul2020
    ...
    public function updateOnQtyChange(){
        //dd($request->all());
        $coupon = Coupon::where('code', \App\Coupon::find(Session::get('coupon_id'))->code)->first();
    ...
    public function order_confirmed(){
        ...
        $order = OrderDetail::where('order_id', Session::get('order_id'))->get();
